        ...
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden md:block">
            <div className="ml-10 flex items-baseline space-x-8">
              {navLinks.map((link) => (
                <a key={link.name} href={link.href} className="text-gray-300 hover:text-primary transition-colors px-3 py-2 rounded-md text-sm font-medium">
                  {link.name}
                </a>
              ))}
            </div>
          ...
          <div className="hidden md:flex items-center gap-4">
            ...
            <div className="relative" ref={langMenuRef}>
              <button 
                ...
                className="text-gray-400 hover:text-white transition-colors flex items-center gap-1 text-sm font-mono px-3 py-2 rounded-lg hover:bg-white/5"
              >
                <Globe size={16} />
              ...
              <AnimatePresence>
                {langMenuOpen && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: -10 }}
                    className="absolute right-0 mt-2 w-48 bg-dark/95 backdrop-blur-xl border border-white/10 rounded-xl shadow-xl overflow-hidden z-50"
                  >
                    <div className="py-2">
                      {languages.map((lang) => (
                        <button
            ...
            <a 
              href="https://mip666.cc"
              target="_blank"
              rel="noopener noreferrer"
              className="bg-white/10 hover:bg-white/20 border border-white/20 text-white px-4 py-2 rounded-full text-sm font-bold transition-all flex items-center gap-2 group"
            >
              {t('nav.launch_app')}
        ...
        {isOpen && (
          ...
          >
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
              ...
              <div className="px-3">
                <p className="text-gray-500 text-xs mb-2 px-1">Language / 语言</p>
              ...
              <a href="https://mip666.cc" target="_blank" rel="noopener noreferrer" className="w-full text-center mt-4 bg-gradient-to-r from-primary to-secondary text-white block px-3 py-3 rounded-lg font-bold">
                {t('nav.launch_app')}
              </a>
            </div>
          </motion.div>
